...
public class RegisterController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        usernameField.setOnKeyPressed(event -> {
            if (event.getCode() == KeyCode.TAB || event.getCode() == KeyCode.ENTER) {
                event.consume(); emailField.requestFocus();
            }
        });
        emailField.setOnKeyPressed(event -> {
            if (event.getCode() == KeyCode.TAB || event.getCode() == KeyCode.ENTER) {
                event.consume(); passwordField.requestFocus();
            }
        });
        passwordField.setOnKeyPressed(event -> {
            if (event.getCode() == KeyCode.ENTER) {
                event.consume(); handleRegister();
            }
        });
    ...
    private void handleRegister() {
        ...
        if (username.isEmpty() || email.isEmpty() || password.isEmpty()) {
            showError("Por favor rellena todos los campos."); return;
        }
        ...
        new Thread(() -> {
            try {
                ...
                Platform.runLater(() -> {
                    try { SceneManager.getInstance().showChat(); }
                    catch (Exception e) { showError("Error al cargar el chat."); }
                });
            } catch (Exception e) {
                Platform.runLater(() -> {
                    showError("Error al crear la cuenta. El email ya existe.");
    ...
    private void goToLogin() {
        try { SceneManager.getInstance().showLogin(); }
